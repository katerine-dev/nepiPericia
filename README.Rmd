...
cnae_base |>
  ...
  ggplot2::ggplot() +
  ggplot2::aes(x = prop, y = natureza, label = pct(prop)) +
  ggplot2::geom_col(fill = "#eccc40", width= 0.8) +
  ggplot2::geom_label(ggplot2::aes(x = prop/2), size = 3) +
  ggplot2::facet_wrap( ~ resultado_da_constatacao, scales = "free_x") +
...
cnae_base |>
  dplyr::filter(!is.na(complementacao_docs)) |> 
  dplyr::group_by(resultado_da_constatacao) |>
  dplyr::count(complementacao_docs) |>
  janitor::adorn_totals() |>
  dplyr::mutate(prop = n / sum(n)*2) |>
  dplyr::mutate(prop = formattable::percent(prop)) |>
  purrr::set_names("Desfecho", "Complementação de Documento", "Quantidade", "%") |>
